...
public class BDD {
  ...
  String url;
   ...
   // fonction pour faire connection au base de donné
public Connection connexionDatabase(){
    try{
        Class.forName("com.mysql.jdbc.Driver");
        connection = DriverManager.getConnection(url, username, password);    
    } catch (Exception e)
    {System.err.println(e.getMessage()); //e.getMessage est pour affichier ou se trouve le probleme exactement err pour
    //afficher le erreur err
            }
            return connection;
    }
 // fonction pour fermer la base de donné
...
// Fonction de requête pour afficher Tous 
...
// Fonction pour affichier Tous avec paramètre ("état")
...
SQL += ") VALUES(";
for (i = 0; i <= contenuTableau.lenght - 1; i++) {
SQL += "'" + contenuTableau[i] + "'";
if (i < contenuTableau.length -1){
SQL += ")";
   }
...
for (i = 0; i <= nomColonne.lenght - 1; i++){
